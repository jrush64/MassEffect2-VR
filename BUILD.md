...
The build runs MSBuild on `src\MELE2VR.vcxproj` (Release|x64) with the mod's own sources
...
Copy these files into `<Steam library>\steamapps\common\Mass Effect Legendary Edition\Game\ME2\Binaries\Win64\`,
next to `MassEffect2.exe`:
...
- `MELE2VR.ini` (see `runtime/MELE2VR.ini` for a starting point)

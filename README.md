...
A 6DOF VR mod for Mass Effect Legendary Edition (ME2), built as a `dxgi.dll` proxy loaded next
to `MassEffect2.exe`. It hooks Direct3D 11 and the game's UE3 render path directly and submits
...
- **Real geometric stereo** - the engine renders the whole frame twice per present, one full
  primary render per eye, so both eyes get the engine's normal lighting and post-processing.
...
- **Per-eye HUD and menus** - panels and text are drawn once per eye so UI sits at the right depth.
  See [`docs/ME2_UI_STEREO_FIX.md`](docs/ME2_UI_STEREO_FIX.md).
...
- **First-person camera** - per-camera-mode first-person offsets with weapon/body visibility handling.
- **Conversation first-person view** - optional first-person camera during dialogue.
- **Meta Quest Link support** - the projection is filled to cover the Meta compositor's own frustum.
...
| `runtime/` | A starting `MELE2VR.ini` |

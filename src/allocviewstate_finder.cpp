...
void TryFindOnce() noexcept
{
    ...
    if (!LocateText())
    {
        ME2VR::Log::Line("[ME2DISC] AllocViewStateFinder: could not locate .text");
        return;
    }
    ...
    ME2VR::Log::Line("[ME2DISC] AllocViewStateFinder: ViewState(+0x46C) store sites=" +
    ...
    for (std::uintptr_t s : stores)
    {
        ...
        ME2VR::Log::Line("[ME2DISC]   store@+" + Hex(s - g_base) +
    ...
    if (targets.size() == 1)
    {
        ME2VR::Log::Line("[ME2DISC] >>> AllocateViewState (likely) = MassEffect2.exe+" +
                         Hex(targets[0] - g_base) + " <<<  (single consistent xref)");
    }
    else if (targets.empty())
    {
        ME2VR::Log::Line("[ME2DISC] AllocViewStateFinder: no rax-store xref found; ctor may move via another reg.");
    }
    else
    {
        std::string line = "[ME2DISC] AllocateViewState candidates:";

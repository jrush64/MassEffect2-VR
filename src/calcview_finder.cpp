...
namespace
{
// FViewportClient::Draw, captured live by the engine probe (2026-06-26 run).
constexpr std::uintptr_t kDrawRva = 0x6D0A00;
...
void BuildCandidates() noexcept
{
    ...
    ME2VR::Log::Line("[ME2DISC] CalcViewFinder: Draw=" + Hex(drawAddr) +
...
void Tick() noexcept
{
    ...
    if (g_started.compare_exchange_strong(expectedStart, true, std::memory_order_acq_rel))
    {
        ...
        g_stub = BuildStub();
        if (g_stub == nullptr) { ME2VR::Log::Line("[ME2DISC] CalcViewFinder: stub alloc FAILED"); g_done = true; return; }
        if (MH_Initialize() == MH_OK) g_minhookReady = true;
        else { ME2VR::Log::Line("[ME2DISC] CalcViewFinder: MH_Initialize FAILED"); g_done = true; return; }
        BuildCandidates();
        return;
    }
    ...
    if (!g_curHooked)
    {
        ...
        {
            ME2VR::Log::Line("[ME2DISC] CalcViewFinder: done. " + std::to_string(g_matchCount) +
    ...
    if (++g_curPresents >= kPresentsPerCandidate)
    {
        if (g_curArgHit != 0)
        {
            ...
            ++g_matchCount;
            ME2VR::Log::Line("[ME2DISC] >>> CalcSceneView CANDIDATE: MassEffect2.exe+" + Hex(rva) +

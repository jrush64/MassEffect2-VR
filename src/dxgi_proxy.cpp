...
void LoadRealDxgi() noexcept
{
    ...
    if (g_realDxgi == nullptr)
    {
        ...
        ME2VR::Log::WindowsError("[ME2DISC] LoadLibraryW real dxgi.dll", error);
        return;
    }
    g_loadResult = S_OK;
    ME2VR::Log::Line("[ME2DISC] loaded real dxgi.dll: " + ME2VR::Log::WideToUtf8(path));
}
...
FARPROC Resolve(const char* name) noexcept
{
    ...
    if (!EnsureLoaded())
    {
        Log::Line(std::string("[ME2DISC] cannot resolve export; real dxgi not loaded: ") + name);
        return nullptr;
    }
    FARPROC proc = GetProcAddress(g_realDxgi, name);
    if (proc == nullptr) Log::WindowsError((std::string("[ME2DISC] missing real dxgi export: ") + name).c_str(), GetLastError());
    return proc;
}

HRESULT MissingExport(const char* name) noexcept
{
    Log::Line(std::string("[ME2DISC] returning failure for missing dxgi export: ") +
...
extern "C" __declspec(dllexport) HRESULT WINAPI CreateDXGIFactory(REFIID riid, void** factory)
{
    ...
    {
        ME2VR::Log::Line("[ME2DISC] CreateDXGIFactory captured factory.");
...
extern "C" __declspec(dllexport) HRESULT WINAPI CreateDXGIFactory1(REFIID riid, void** factory)
{
    ...
    if (SUCCEEDED(hr) && factory != nullptr && *factory != nullptr)
    {
        ME2VR::Log::Line("[ME2DISC] CreateDXGIFactory1 captured factory.");
        ME2VR::D3DCapture::TryInstallFactoryHooks(*factory, riid);
    }
    return hr;
}
...
extern "C" __declspec(dllexport) HRESULT WINAPI CreateDXGIFactory2(UINT flags, REFIID riid, void** factory)
{
    ...
    if (SUCCEEDED(hr) && factory != nullptr && *factory != nullptr)
    {
        ME2VR::Log::Line("[ME2DISC] CreateDXGIFactory2 captured factory.");
        ME2VR::D3DCapture::TryInstallFactoryHooks(*factory, riid);
    }
    return hr;
}
